...
\geometry{a4paper,left=1.5cm,right=1.5cm,top=0.65cm,bottom=0.65cm} % 修改页边距，使得内容在一页范围内
\begin{document}
...
%   \item LeetCodeOJ Solutions, \textit{https://github.com/hijiangtao/LeetCodeOJ}
  ...
  \item NOIP普及组全国一等奖（广东省第7） \& NOIP提高组到达除广东省以外其它省份的全国一等奖分数线
...
\begin{itemize}[parsep=0.2ex]
  ...
  \item \textbf{热情参与开发项目和开源}：曾编写电影推送系统(C, WinAPI; 图文界面和操作, 电影介绍, 不同情景电影推荐) \, ; \, 带有用户操作功能的文章阅读网页(Vue.js, HTTP, Django数据库; 团队协作; 用户和文章增删查改,用户对文章的点赞、收藏和评论,一个用户撰写、认可的所有文章) \, ; \, 地铁在线订票系统(Java, JLayeredPane, Algorithm; 多层结构、图缩放和多种点击方式, 点距离算法设计)。均已GitHub开源。
